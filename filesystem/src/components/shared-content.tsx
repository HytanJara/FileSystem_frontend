...
import { listarCarpetas, compartirCarpeta } from "@/api/carpeta/folder"
import { authService } from "@/api/login/auth"

export function SharedContent() {
  const [archivos, setArchivos] = useState<any[]>([])
  const [carpetas, setCarpetas] = useState<any[]>([])
  const [currentPath, setCurrentPath] = useState<string>("compartidos")
  ...
  useEffect(() => {
    const cargarContenido = async () => {
      try {
        ...
        console.error("Error al cargar contenido compartido:", error)
      }
    }
    cargarContenido()
  }, [currentPath])
  ...
  const handleAtras = () => {
    if (currentPath === "compartidos") return
    const partes = currentPath.split("/")
    partes.pop()
    setCurrentPath(partes.join("/") || "compartidos")
  }
            ...
            {archivos.length > 0 ? (
              ...
              <div className="col-span-full text-center py-12">
                ...
                <p className="text-gray-500">
                  No hay archivos ni carpetas compartidas actualmente
                </p>
              </div>
            )}
          </div>
